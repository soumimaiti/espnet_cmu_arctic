...
if [[ ${pretrain_stage} == "true" ]]; then
    ...
    valid_set=${spk}_dev
    test_sets=${spk}_eval
    #train_config=conf/tuning/train_transformer_all2.yaml
    train_config=conf/tuning/train_transformer_all_sid.yaml
...
if [[ ${adapt_stage} == "true" ]]; then
    ...
    valid_set=${spk}_dev
    test_sets=${spk}_eval
    train_config=conf/tuning/finetune_transformer_2.yaml
    opts="--audio_format wav --local_data_opts ${spk} "
    ./tts.sh \
        --lang en \
        --feats_type raw \
        ...
        --g2p "${g2p}" \
        --train_config "${train_config}" \
        --train_args "--init_param exp/tts_pretrain_transformer/train.loss.best.pth" \
        --inference_config "${inference_config}" \
        --train_set "${train_set}" \
        --valid_set "${valid_set}" \
        --test_sets "${test_sets}" \
